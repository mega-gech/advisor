...
class Assignment extends BaseModel {
    private string $table = 'advisor_assignments';
    ...
    public function isStudentAssignedToAdvisor(int $studentId, int $advisorId): bool {
        $stmt = $this->conn->prepare(
            'SELECT 1 FROM ' . $this->table . '
             WHERE advisor_id = :aid AND student_id = :sid AND is_active = 1 LIMIT 1'
        );
        $stmt->execute([':aid' => $advisorId, ':sid' => $studentId]);
        return (bool) $stmt->fetch();
    }

    public function assignStudent(): bool {
    ...
    public function getAssignedAdvisor($student_id) {
        $query = 'SELECT adv.id as advisor_id, adv.name as advisor_name, adv.email as advisor_email 
                  FROM ' . $this->table . ' a
    ...
    public function getAssignedStudents($advisor_id) {
        $query = 'SELECT s.id as student_id, s.name as student_name, s.email as student_email, s.student_number
                  FROM ' . $this->table . ' a
